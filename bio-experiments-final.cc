...
void TrainDecisionNode(const vector<vector<double>>& data, const vector<int>& labs, TreeNode& node){
    ...
    for(int feature_id = 0; feature_id < NUM_FEATURES; feature_id++){
        ...
        vector<double> feature_values;//特徴量を格納する配列
        for(int i = 0; i < n; i++){
        ...
        for(int p = 1; p <= 99; p++){
            int pos_idx = (int)((double)p / 100.0 * (n - 1));//パーセンタイルは,全体の1%目の値,2%目の値,...と値をとってくること
            double threshold = feature_values[pos_idx];

            vector<int> left_labs, right_labs;
            for(int j = 0; j < n; j++){
                if(data[j][feature_id] <= threshold){
                    left_labs.push_back(labs[j]);//閾値より低いものは,左
                }else{
                    right_labs.push_back(labs[j]);//閾値より高いものは,右
                }
            }
...
// 深さ2の決定木の学習←基本的には,node1を繰り返すだけ。
void TrainDecisionTree(const vector<vector<double>>& data, const vector<int>& labs, vector<TreeNode>& p_tree){//構造体配列として,[0]は親木, [1]は左の子木, [2]は右の子木
    // 親ノード(p_tree[0])を学習
    TrainDecisionNode(data, labs, p_tree[0]);

    // 親ノードでデータを左右に分割
    vector<vector<double>> left_data, right_data;
    vector<int>            left_labs, right_labs;
    for(int i = 0; i < (int)data.size(); i++){
        if(data[i][p_tree[0].feature_id] <= p_tree[0].threshold){
    ...
    // 左子ノード(p_tree[1])・右子ノード(p_tree[2])を学習
    TrainDecisionNode(left_data,  left_labs,  p_tree[1]);
    TrainDecisionNode(right_data, right_labs, p_tree[2]);
}
...
void PrintMetrics(int tp, int fp, int fn, int tn){
    ...
    cout << fixed << setprecision(4);//4桁表示
...
void Evaluation(const TreeNode& node, const vector<vector<double>>& test_data, const vector<int>& test_labels){
    ...
    for(int i = 0; i < (int)test_data.size(); i++){
        if(test_data[i][node.feature_id] <= node.threshold){
            pred = node.left_class_id;
        }else{
            pred = node.right_class_id;
        }
...
// 深さ2の評価
void Evaluation(const vector<TreeNode>& p_tree, const vector<vector<double>>& test_data, const vector<int>& test_labels){
    int tp = 0, fp = 0, fn = 0, tn = 0;
    for(int i = 0; i < (int)test_data.size(); i++){
        int pred;
        if(test_data[i][p_tree[0].feature_id] <= p_tree[0].threshold){
            const TreeNode& child = p_tree[1];
            if(test_data[i][child.feature_id] <= child.threshold){
                pred = child.left_class_id;
            } else {
                pred = child.right_class_id;
            }
        }else{
            const TreeNode& child = p_tree[2];
            if(test_data[i][child.feature_id] <= child.threshold){
                pred = child.left_class_id;
            } else {
                pred = child.right_class_id;
